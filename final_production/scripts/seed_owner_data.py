...
def seed_owner_data(file_path="data/owner_interactions.json", batch_size=2000):
    ...
    try:
        ...
        if batch:
            ...
            db.commit()
            count += len(batch)
            db_logger.info(f"Seeded total {count}/{len(interactions_data)} owner records successfully.")
            
    except Exception as e:
        db.rollback()
        db_logger.error(f"Error seeding owner data: {e}")
        sys.exit(1)
    finally:
        db.close()

...
from prompts.branding_prompt import get_branding_prompt

def branding_node(state: AgentState) -> dict:
    ...
    if is_ar:
        lang_rule = "You MUST write the entire response in elegant, professional standard Arabic (العربية الفصحى المهنية الملائمة لرواد الأعمال في العالم العربي). Do NOT reply in English."
    else:
        lang_rule = "You MUST write the entire response in English. Do NOT reply in Arabic."
        
    system_prompt = get_branding_prompt(
